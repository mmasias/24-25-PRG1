...
class Adivinacion1 {
    public static void main(String[] args) {
        ...
        System.out.println("Estoy pensando en  " + NUMERO_MINIMO + " y " + NUMERO_MAXIMO + "");
        System.out.println("Tienes " + NUMERO_INTENTOS + " oportunidades para adivinarlo");

        while (estaJugando) {
            turno++;
            System.out.print("Intento " + turno + ": Pon un número: ");
            int numeroUsuario = scanner.nextInt();

            if (numeroUsuario == numeroOrdenador) {
                System.out.println("Enhorabuena,has adivinado el número en el intento " + turno + ".");
                estaJugando = false;  
            } else {
                if (numeroUsuario < numeroOrdenador) {
